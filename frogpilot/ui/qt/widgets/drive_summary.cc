...
void FrogPilotDriveSummary::showEvent(QShowEvent *event) {
  ...
  if (displayRandomEvents) {
    ...
    int experimental_time = diffDouble("ExperimentalModeTime");
    int override_time = diffDouble("OverrideTime") - diffDouble("AOLTime");
    int tracked_time = diffDouble("TrackedTime");

    engagementValue->setText(QLocale().toString((tracked_time > 0) ? ((tracked_time - override_time) * 100 / tracked_time) : 0) + "%");
    experimentalModeTimeValue->setText(QLocale().toString((tracked_time > 0) ? (experimental_time * 100 / tracked_time) : 0) + "%");
    frogPilotMetersValue->setText(format_distance(diffDouble("FrogPilotMeters")));
    trackedTimeValue->setText(format_time(tracked_time));
  }
}
