...
FrogPilotVehiclesPanel::FrogPilotVehiclesPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  hardwareDetected = new LabelControl(tr("Hardware Detected"), tr("None"));
  settingsList->addItem(hardwareDetected);
...
void FrogPilotVehiclesPanel::showEvent(QShowEvent *event) {
  ...
  if (detected.isEmpty()) {
    hardwareDetected->hide();
  } else {
    hardwareDetected->setText(detected.join(", "));
    hardwareDetected->show();
  }
